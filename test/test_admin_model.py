...
def test_search(client):
    ...
    response = client.get('/Flower', params={'search': 'Роза'})
    assert response.status_code == 200
    response_json = response.json()
    records = response_json.get('records')
    assert records != None
...
def test_offset(client):
    response = client.get('/Flower', params={'offset': 11})
    assert response.status_code == 200
    response_json = response.json()
    records = response_json.get('records')
    assert records != None
...
def test_limit(client):
    response = client.get('/Flower', params={'limit': 3})
    assert response.status_code == 200
    response_json = response.json()
    records = response_json.get('records')
    assert records != None
...
def test_limit_unreasonable_values_returns_default(client, limit):
    response = client.get('/Flower', params={'limit': limit})
    assert response.status_code == 200
    response_json = response.json()
    records = response_json.get('records')
    assert records != None
...
def test_order_default(client):
    response = client.get('/Flower', params={'order': 'id'})
    assert response.status_code == 200
    response_json = response.json()
    records = response_json.get('records')
    assert records != None
...
def test_order_asc(client):
    response = client.get('/Flower', params={'order': 'id', 'order_type': 'asc'})
    assert response.status_code == 200
    response_json = response.json()
    records = response_json.get('records')
    assert records != None
...
def test_order_desc(client):
    response = client.get('/Flower', params={'order': 'id', 'order_type': 'desc'})
    assert response.status_code == 200
    response_json = response.json()
    records = response_json.get('records')
    assert records != None
...
def test_flower_filter_by_name(client):
    response = client.get('/Flower', params={'filters[name]': 'Роза'})
    assert response.status_code == 200
    response_json = response.json()
    records = response_json.get('records')
    assert len(records) == 1
...
def test_flower_filter_by_id_gte(client):
    response = client.get('/Flower', params={'filters[id__gte]': 3})
    assert response.status_code == 200
    response_json = response.json()
    records = response_json.get('records')
    assert len(records) == 8
...
def test_flower_filter_by_id_gt(client):
    response = client.get('/Flower', params={'filters[id__gt]': 3})
    assert response.status_code == 200
    response_json = response.json()
    records = response_json.get('records')
    assert len(records) == 8
...
def test_flower_filter_by_id_lt(client):
    response = client.get('/Flower', params={'filters[id__lt]': 3})
    assert response.status_code == 200
    response_json = response.json()
    records = response_json.get('records')
...
def test_flower_filter_by_id_lte(client):
    response = client.get('/Flower', params={'filters[id__lte]': 3})
    assert response.status_code == 200
    response_json = response.json()
    records = response_json.get('records')
    assert len(records) == 3
...
def test_flower_filter_by_id_ne(client):
    response = client.get('/Flower', params={'filters[id__ne]': 3})
    assert response.status_code == 200
    response_json = response.json()
    records = response_json.get('records')
    assert len(records) == 8
...
def test_flower_filter_by_id_eq(client):
    response = client.get('/Flower', params={'filters[id__eq]': 3})
    assert response.status_code == 200
    response_json = response.json()
    records = response_json.get('records')
    assert len(records) == 1
